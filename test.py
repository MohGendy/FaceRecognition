...
# Load model and other paths (as in your original code)
# ...
...
# Set frame skip interval (e.g., process every 5th frame)
frame_skip = 100
frame_count = -1
...
while True:
    ...
    if frame_count % frame_skip == 0:
        ...
        for (x_face, y_face, w_face, h_face) in faces:
            # Your existing face recognition code (as in your original code)
            # ...
            for (x_face, y_face, w_face, h_face) in faces:
                ...
                for i in range(len(embeddings)):
                    dist.append(distance.euclidean(l2_normalize(model.predict(prewhiten(
                        cv2.resize(frame[y_face:y_face + h_face, x_face:x_face + w_face], (160, 160)).reshape(-1, 160, 160,
                                                                                                              3)))),
